...
Route colors distinguish diagnostic and first-contact movement, treatment movement, follow-up movement, and conditional or return routes. Arrowheads show sequence direction. Selected stations receive numbered badges based on the directed pathway order. The map constructs one patient treatment route at a time: selecting an activity on a competing treatment branch replaces selected activities on the previous branch, while sequential or combined activities inside the same branch can remain selected together. Selecting a station opens its activity definition, ECO-PATH module assignment, and the physical LCI inputs inherited from its mapped building blocks.

Two assessment purposes are available. **Complete pathway assessment** requires the selected activities to form one connected directed route between the declared start and endpoint. The browser results, report export, scenario export, and direct Brightway calculation remain disabled if the route starts or stops inside the declared boundary, omits a connecting activity, contains a disconnected selection, or includes competing exits from the same decision point. **Partial segment exploration** accepts one activity or a connected subset for component-level analysis. Its outputs are explicitly labeled as partial segment results and must not be reported as a complete patient-pathway footprint. Switching between the two purposes starts a new assessment scenario and clears the previous activity selection and boundary state.

The browser calculator uses the prototype formula `impact = quantity x unit impact result` for a selected EF category. A unit impact result is the LCIA score for one reference unit of a building block, not an EF characterization factor for an elementary flow. The browser values are synthetic demonstration data. In a formal model, Brightway calculates these unit results from reviewed foreground processes, linked background datasets, and the selected EF method. The calculator sums active building blocks into one pathway result and optionally multiplies this result by the cohort size. The cohort defaults to `1`, so the initial total corresponds to one declared functional unit. Changing the cohort size scales only the cohort total and does not redefine the functional unit.

## LCI inputs, activity data, and impact assessment

ECO-PATH keeps three data roles separate. Physical LCI inputs are quantified products, energy, transport services, water, materials, waste-treatment services, and direct emissions. These exchanges belong in the foreground inventory. Activity data such as operating-room hours, room-hours, equipment-hours, visits, fractions, cycles, and bed-days define building-block quantities or allocate shared physical inputs. Allocation rules document how a department or hospital total is assigned to a building block. Activity data and allocation rules are model parameters, not environmental exchanges, and they do not receive characterization factors.
...
The collapsible **Sensitivity and improvement explorer** ranks the five largest positive building-block contributions for the selected impact category. The ranking does not change all five entries automatically. No contributor is selected by default. Users can check one or more rows and test either a unit-impact reduction or a whole-number activity count. Every checked contributor has an independent control. The master slider applies only to unit-impact scenarios and overwrites the individual percentages of currently checked contributors with one shared value. In count mode, users enter an integer from `1` to the baseline count or use **Set to 1**. A count of `0` is treated as exclusion of the activity or building block and must be represented through pathway or boundary selection, not count sensitivity.
...
The static browser application cannot run Brightway or distribute licensed background databases. For direct calculation, run the explorer through the local companion service in the Python environment that has access to the intended Brightway project:
...
3. Complete `brightway/mapping.local.json` once. Use existing foreground activity database/code identifiers and exact installed EF 3.1 no long-term method tuples. In Brightway terminology, these foreground activities are process nodes and commonly correspond to ISO unit processes. The explorer does not create their technosphere or biosphere exchanges. Do not use name-based fuzzy matching for formal results.
...
The LCA module registry is shared across specialties so that the same resource-bearing modeling zones can be reused and compared. A pathway does not need to activate every module. EM04 and EM05 have been retired from the LCA registry. Joint-clinic, MDT/MDO, eligibility, and treatment-choice nodes remain part of the clinical pathway topology, but they do not create LCA modules, building blocks, or calculation rows. EM10 and EM11 remain optional support modules. EM10 is the single owner of patient and caregiver travel, so travel must not be embedded in coefficients for EM01-EM09 or EM11.
...
The prototype separates clinical pathway topology from LCA accounting. Decision points, joint-clinic logic, and MDT/MDO review remain visible so clinicians can inspect route logic and activity sequence. A display-only eye control can hide or show decision points without changing selected activities, route validation, module activation, quantities, or results. These clinical-logic elements are not LCA modules and do not generate decision-related building blocks or calculation rows. If the logic triggers a separate visit, laboratory test, imaging examination, treatment, or procedure, that physical activity must be modeled in its resource-bearing module. Patient and caregiver travel are modeled exclusively in EM10. If EM10 is outside the selected boundary, travel is excluded from the scenario and is not reassigned to another clinical module. Human labor is not characterized as an environmental resource flow; activity duration may still be used as an allocation driver for documented electricity, heating, cooling, materials, or service inputs. This is a foreground boundary rule, not evidence that decision processes have zero environmental impact. The exclusion must be documented and reviewed during formal LCA modeling.
